...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Generate an App Engine app.yaml and optionally deploy this application.')
    ...
    if config_type == 'production':
        ...
        if not stdin and args.generate_production <= 0:
            ...
            print
            "Either tag the current commit as 'version-VERSION' or provide " + \
        ...
        if args.generate_production > 0:
            ...
            try:
                ...
                print
                "ERROR: Could not determine version number from tag", stdin
                sys.exit(1)
    else:
        print
    ...
    print
    "Generating configuration for version", version
    ...
    if deploy_app:
        if os.environ.get('OAUTH_REFRESH_TOKEN'):
            ...
            if return_code == 0:
                ...
                print
                "Could not deploy application. Running appcfg rollback..."
                subprocess.call(ROLLBACK_COMMAND, shell=True)
        else:
            print
            "No credentials found for deployment."
